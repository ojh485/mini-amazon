...
SELECT pg_catalog.setval('public.carts_id_seq', (SELECT MAX(id)+1 FROM Carts), false);

\COPY Sells FROM 'Sells.csv' WITH DELIMITER ',' NULL '' CSV
SELECT pg_catalog.setval('public.sells_id_seq', (SELECT MAX(id)+1 FROM Sells), false);

\COPY Reviews FROM 'Reviews.csv' WITH DELIMITER ',' NULL '' CSV
SELECT pg_catalog.setval('public.reviews_id_seq', (SELECT MAX(id)+1 FROM Reviews), false);

\COPY Ratings FROM 'Ratings.csv' WITH DELIMITER ',' NULL '' CSV
SELECT pg_catalog.setval('public.ratings_id_seq', (SELECT MAX(id)+1 FROM Ratings), false);